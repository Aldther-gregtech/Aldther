...
query = """
SELECT RazmKosguGou.[Статья] AS статья_1, RazmKosguGou.[лицевой], ORGb.[ИНН], RazmKosguGou.[ПФХД], 
RazmKosguGou.[TO], VrGOU.[Статья] AS Косгу, RazmKosguGou.[КодКЗ] AS КЗ, RazmKosguGou.[КВР], RazmKosguGou.[N] AS n,
RazmKosguGou.[00], RazmKosguGou.[83], RazmKosguGou.[90], RazmKosguGou.[в работе 00, 30,39,99] AS в_работе, RazmKosguGou.[зарегистрировано 00, 30,39,99] AS зарегистрировано
FROM (RazmKosguGou
INNER JOIN VrGOU ON RazmKosguGou.[N] = VrGOU.[N])
INNER JOIN ORGb ON RazmKosguGou.[лицевой] = ORGb.[Лицевой счет]
"""
...
# Определение модели для таблицы Gou в PostgreSQL
...
try:
    df.to_sql('Gou', engine, schema='Online', if_exists='replace', index=False)  # Заменяем существующую таблицу
    print("Загрузка Онлайн ГОУ успешно завершена")
